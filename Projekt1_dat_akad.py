...
sirka_sloupce_occurences = max(nejvyssi_pocet_znaku)
...
for i in (range(1, max_delka_slova + 1)):
    vyskyt_poctu_znaku = len([word for word in upraveny_text if len(word) == i])
    print(f"{str(i).rjust(4, ' ')}|"
          f"{('*' * vyskyt_poctu_znaku).ljust(sirka_sloupce_occurences + 1)}|"
          f"{str(vyskyt_poctu_znaku).ljust(3, ' ')}")
