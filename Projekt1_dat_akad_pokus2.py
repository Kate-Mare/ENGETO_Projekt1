...
upraveny_text = (TEXTS[text_volba-1].replace(",", "")
                .replace(".", "")
                .strip()
                .split())
...
# seznam jednotlivých výskytů znaků podle jejich délek
vyskyt_podle_poctu_znaku = [
    len([word for word in upraveny_text if len(word) == i])
    for i in range(1, max_delka_slova + 1)
]
sirka_sloupce_occurences = max(vyskyt_podle_poctu_znaku)
# určení správné šířky sloupce, aby se tam vešly všechny "*"
print(f"{'LEN'.rjust(4)}|"
      f"{'OCCURENCES'.center(sirka_sloupce_occurences + 1, ' ')}|"
      f"{'NR.'.ljust(3)}")
for i in range(1, max_delka_slova + 1):
    pocet_vyskytu = vyskyt_podle_poctu_znaku[i - 1]
    print(f"{str(i).rjust(4, ' ')}|"
          f"{('*' * pocet_vyskytu).ljust(sirka_sloupce_occurences + 1)}|"
          f"{str(pocet_vyskytu).ljust(3, ' ')}")
